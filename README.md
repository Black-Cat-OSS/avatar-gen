...
**✨ Автоматическая настройка:** При запуске автоматически инициализируется
TypeORM подключение к базе данных и выполняется синхронизация схемы.
...
- **База данных**: SQLite/PostgreSQL + TypeORM
...
npm run test         # Запуск тестов
npm run typeorm:generate  # Генерация TypeORM миграций
...
Настройка выполняется через YAML файлы в папке `backend/`:
