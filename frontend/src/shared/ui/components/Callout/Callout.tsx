...
export const Callout = ({
  ...
  const getIcon = () => {
    switch (type) {
      case 'info':
        return (
          <div
            className={`${styles.calloutIcon} flex-shrink-0 w-5 h-5`}
          >
  ...
  return (
    <div
      className={`${styles.callout} ${getTypeClass()} ${className}`}
    >
      ...
      {subtitle && (
        <p className={styles.calloutSubtitle}>{subtitle}</p>
      )}
      {children && (
        <div
          className={`${styles.calloutContent} ${getVariantClass()}`}
        >
          {children}
        </div>
      )}
