...
export class MessageBuffer {
  ...
  addVerticalLine(
    ...
    if (column >= 0 && column < this.width) {
      for (
        ...
        line <= Math.min(endLine, this.height - 1);
        line++
      ) {
        this.buffer[line][column] = char
      }
    ...
    return this.buffer
      ...
      .reduce((acc, line) => {
        ...
        if (trimmedLine.length > 0) {
          return (
            acc + `${color}${trimmedLine}${resetColor}\n`
          )
        }
        return acc + '\n'
      }, '')
  ...
  slice(
    ...
    const slicedBuffer = new MessageBuffer(
      slicedWidth,
      slicedHeight,
    )
    ...
    for (let line = 0; line < slicedHeight; line++) {
      ...
      if (sourceLine >= 0 && sourceLine < this.height) {
        for (
          ...
          if (
            sourceColumn >= 0 &&
            sourceColumn < this.width
          ) {
            slicedBuffer.setChar(
              column,
              line,
              this.buffer[sourceLine][sourceColumn],
            )
          }
        }
      }
   ...
   */
  static concatHorizontal(
    ...buffers: MessageBuffer[]
  ): MessageBuffer {
    const totalWidth = buffers.reduce(
      (sum, buffer) => sum + buffer.width,
      0,
    )
    const maxHeight = Math.max(
      ...buffers.map((buffer) => buffer.height),
    )
    ...
    buffers.forEach((buffer) => {
      for (
        let line = 0;
        line < buffer.height && line < maxHeight;
        line++
      ) {
        for (
          let column = 0;
   ...
   */
  static concatVertical(
    ...buffers: MessageBuffer[]
  ): MessageBuffer {
    const maxWidth = Math.max(
      ...buffers.map((buffer) => buffer.width),
    )
    const totalHeight = buffers.reduce(
      (sum, buffer) => sum + buffer.height,
      0,
    )
    ...
    buffers.forEach((buffer) => {
      for (let line = 0; line < buffer.height; line++) {
        for (
          let column = 0;
          column < buffer.width && column < maxWidth;
          column++
        ) {
          result.setChar(
            column,
